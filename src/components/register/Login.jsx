...
function Login() {
  ...
  return (
    <div className={`max-w-md relative mt-[4rem] sm:mt-[7rem] md:mt-[11rem] xl:mt-[11rem] mx-auto p-8 rounded-lg shadow-lg ${mode === 'light' ? 'bg-white' : 'bg-gray-800 shadow-gray-900'}`}>
      <h2 className={`text-3xl font-bold mb-6 text-center ${mode === 'light' ? 'text-gray-800' : 'text-gray-200'}`}>Login</h2>
      ...
      <div className={`mt-4 text-center ${mode === 'light' ? 'text-gray-800' : 'text-gray-300'}`}>
        ...
        <Link to="/register" className={`hover:underline ${mode === 'light' ? 'text-blue-500' : 'text-blue-400'}`}>
          Register
        </Link>
      </div>
    </div>
